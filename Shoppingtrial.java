...
public class Shoppingtrial {
    ...
    private static final String PASS = "dhyan";
    ...
    private static void processItemAddition() throws Exception {
    ...
    private static void processPurchase() throws SQLException {
        ...
        // --- EDGECASE IMPLEMENTATION: Insufficient Balance ---
    ...
    private static void renderBasket(int basketIdentifier) throws SQLException {
        ...
        System.out.println("+------+--------------------------+-----------+-----+------------+");
        System.out.printf("|                                           TOTAL | %10.2f |%n", finalAmount);
        System.out.println("+-------------------------------------------------+------------+");
    }
    ...
    private static void renderInvoice(int invoiceIdentifier) throws SQLException {
        ...
        try (PreparedStatement prepStmt17 = dbConnection.prepareStatement(invoiceTotSql)) {
            ...
            resSet12.next();
            System.out.println("+-----------+--------------------------+-----+----------+");
            System.out.printf( "|                                   TOTAL  | %8.2f |%n",
                               resSet12.getBigDecimal("total_amount"));
            System.out.println("+------------------------------------------+----------+");
        }
    }

    private static void displayInvoices() throws SQLException {
        int customerIdentifier = askForCustomerIdentifier();
        if (customerIdentifier < 0) return;
        String listInvoicesSql = "SELECT bill_id, total_amount, billed_at FROM bill " +
                     "WHERE user_id = ? ORDER BY billed_at DESC";
        System.out.println("\n+-- Past Bills ---------------------------------------+");
        try (PreparedStatement prepStmt18 = dbConnection.prepareStatement(listInvoicesSql)) {
            prepStmt18.setInt(1, customerIdentifier);
            ResultSet resSet13 = prepStmt18.executeQuery();
            boolean hasAnyInvoices = false;
            while (resSet13.next()) {
                hasAnyInvoices = true;
                System.out.printf("| Bill #%-4d |  %12.2f  |  %-21s|%n",
                    resSet13.getInt("bill_id"),
                    resSet13.getBigDecimal("total_amount"),
                    resSet13.getTimestamp("billed_at"));
            }
            if (!hasAnyInvoices) System.out.println("|  No past bills found.                               |");
        }
        System.out.println("+-----------------------------------------------------+");
        System.out.print("View detailed bill? Enter bill ID (or 0 to skip): ");
        int selectedInvoice = parseStringToIntOrNegative(inputScanner.nextLine());
        if (selectedInvoice > 0) {
            // Check if the bill exists AND belongs to the current user
            String verifyBillSql = "SELECT bill_id FROM bill WHERE bill_id = ? AND user_id = ?";
            try (PreparedStatement verifyStmt = dbConnection.prepareStatement(verifyBillSql)) {
                verifyStmt.setInt(1, selectedInvoice);
                verifyStmt.setInt(2, customerIdentifier);
                ResultSet verifyRs = verifyStmt.executeQuery();
                
                if (verifyRs.next()) {
                    // Bill is valid, go ahead and render it
                    renderInvoice(selectedInvoice);
                } else {
                    // Bill not found, handle the error gracefully
                    System.out.println("[!] Invalid Bill ID or bill does not belong to you.");
                }
            }
        }
    }
